...
<html lang="en" xmlns="http://www.w3.org/1999/xhtml">
	...
	<div class="content" align=center>
		...
		<br>
			...
			<tr><th class="exportmember"><a href="availabilityEdit.php"> Update </a></th><th  class="newday" colspan=8>Tuesday <? echo $week_start; ?></th><th  class="newday" colspan=8>Wednesday <? echo $day1; ?></th><th  class="newday" colspan=8>Thursday <? echo $day2; ?></th><th  class="newday" colspan=8>Friday <? echo $day3; ?></th><th class="newday" colspan=8>Saturday <? echo $day4; ?></th><th class="newday" colspan=8>Sunday <? echo $day5; ?></th><th  class="newday" colspan=8>Monday <? echo $day6; ?></th></tr>
			...
			echo "<tr> <td> Available </td>";
				$DayCount = 0;
				$DayLimit = 7;
				for ($x = $DayCount; $x < $DayLimit; $x++) {
					
					$TimeSlotCount = $Availability->CountAvailabilityByDay($week,$DayCount,1, $IDTeam);
					if($TimeSlotCount < 3)
					{ echo '<td class="newday red">' . $TimeSlotCount . '</td>'; }
					else
					{ echo '<td class="newday green">' . $TimeSlotCount . '</td>'; }
					
					$TimeSlotCount = $Availability->CountAvailabilityByDay($week,$DayCount,2, $IDTeam);
					if($TimeSlotCount < 3)
					{ echo '<td class="red">' . $TimeSlotCount . '</td>'; }
					else
					{ echo '<td class="green">' . $TimeSlotCount . '</td>'; }
					
					$TimeSlotCount = $Availability->CountAvailabilityByDay($week,$DayCount,3, $IDTeam);
					if($TimeSlotCount < 3)
					{ echo '<td class="red">' . $TimeSlotCount . '</td>'; }
					else
					{ echo '<td class="green">' . $TimeSlotCount . '</td>'; }
				
					$TimeSlotCount = $Availability->CountAvailabilityByDay($week,$DayCount,4, $IDTeam);
					if($TimeSlotCount < 3)
					{ echo '<td class="red">' . $TimeSlotCount . '</td>'; }
					else
					{ echo '<td class="green">' . $TimeSlotCount . '</td>'; }
					
					$TimeSlotCount = $Availability->CountAvailabilityByDay($week,$DayCount,5, $IDTeam);
					if($TimeSlotCount < 3)
					{ echo '<td class="red">' . $TimeSlotCount . '</td>'; }
					else
					{ echo '<td class="green">' . $TimeSlotCount . '</td>'; }
				
					$TimeSlotCount = $Availability->CountAvailabilityByDay($week,$DayCount,6, $IDTeam);
					if($TimeSlotCount < 3)
					{ echo '<td class="red">' . $TimeSlotCount . '</td>'; }
					else
					{ echo '<td class="green">' . $TimeSlotCount . '</td>'; }
				
					$TimeSlotCount = $Availability->CountAvailabilityByDay($week,$DayCount,7, $IDTeam);
					if($TimeSlotCount < 3)
					{ echo '<td class="red">' . $TimeSlotCount . '</td>'; }
					else
					{ echo '<td class="green">' . $TimeSlotCount . '</td>'; }
				
					$TimeSlotCount = $Availability->CountAvailabilityByDay($week,$DayCount,8, $IDTeam);
					if($TimeSlotCount < 3)
					{ echo '<td class="red">' . $TimeSlotCount . '</td>'; }
					else
					{ echo '<td class="green">' . $TimeSlotCount . '</td>'; }
		...
		echo "</tr>";
			...
			if($IDTeam > 0) 
			{
				$stmtMembers = $db->prepare("SELECT * FROM tblmembers where IDTeam=? order by Name");
				$stmtMembers->bind_param("i", $IDTeam);
			}
			else
			{
				$stmtMembers = $db->prepare("SELECT * FROM tblmembers order by Name");
			}
			...
			if($resultMembers->num_rows != 0) 
			{
				...
				while($rowMember = $resultMembers->fetch_assoc()) 
				{
						...
						if($MemberID > 0)
						 ...
						 echo "</tr><tr>";
						 $DayCount = 0;
						 echo "<td><b>" . $Availability->GetMemberName($MemberID) . "</b></td>";
					...
					if($result->num_rows != 0) 
					{
						...
						while($row = $result->fetch_assoc()) {
							...
							if($DayCount == $DayID)
							{
								...
								if($TimeSlot1 == 1)
								{echo '<td class="green newday">Y</td>';}
								else
								{echo '<td class="red newday">N</td>';}
							
								if($TimeSlot2 == 1)
								{echo '<td class="green">Y</td>';}
								else
								{echo '<td class="red">N</td>';}
